...
function checkRequest(req: Request): string {
  if (!req) {
...
function getFileName(url: string, ext: string) {
  ...
  return qsObj.get(KEY_FILE_NAME_LC) || qsObj.get(KEY_FILE_NAME_CC) ||
    randomFileName;
}
...
async function postProcessMFD(data: any, options: ParserOptions) {
  for (const k in data.files) {
    ...
    if (!file.path && options.saveBodyToFile === false) {
      ...
      const fileData = await saveIntoFile(
        { ext } as ContentMeta,
        file.content,
        "",
      ...
      const newPath = (await Deno.realPath(options.saveFilePath || "./")) +
        "/" + getRandomFileName(ext);
  ...
  const jsonFileData: FileData = await saveIntoFile(
    SupportedContentTypeMetadata[PARSER_TYPES.JSON],
    rawData,
    "",
    options.saveFilePath,
  );
...
async function prepareResponse(
  ...
  if (
    (ctMeta.parser === PARSER_TYPES.UNKNOWN &&
      options.unknownAsText === true) ||
  ...
  if (ctMeta.parser === PARSER_TYPES.MFD) {
    if (decodedObj.decoded[INTERNAL_MFD_FILE_KEY]) {
      ret[ResponseTypes.RESP_TYPE_FILE] =
        decodedObj.decoded[INTERNAL_MFD_FILE_KEY];
  ...
  const fileData: FileData = await saveIntoFile(
    ctMeta,
    decodedObj.raw,
    url,
    options.saveFilePath,
...
export type { FileData, ParserOptions } from "./metadata.ts";
