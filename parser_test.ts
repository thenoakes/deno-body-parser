...
import { Buffer, BufReader } from "https://deno.land/std@0.122.0/io/buffer.ts";
...
const HDR_VAL_UNKNOWN_TYPE = "x-unknown-type",
  ...
  OPTIONS_SAVE_BODY_TO_FILE_TO_PATH = Object.assign(
    {},
    OPTIONS_SAVE_BODY_TO_FILE,
    { saveFilePath: TEMP_DIR },
  ),
  ...
  XML_STR = JSON.stringify({
    tag: "tag content",
    tag2: "another content",
    tag3: { insideTag: "inside content", emptyTag: null },
  });
...
async function addBody(
  ...
  if (ext) {
    ...
    if (ct !== ParserMeta.MIME_CONTENT_TYPES.MULTIPART_FORM_DATA) {
      ...
      // TODO: Get content-length. MFD is broken until this is fixed.
...
async function fileAsserts(
  files: Record<string, ParserMeta.FileData>,
  initPath: string,
  ext: string,
  compareWithOrig: boolean = false,
) {
...
async function dataAsserts(ret: any, data: string | undefined, ext?: string) {
  ...
  if (!ext) {
    ...
    const rcvdDataStr = JSON.stringify(ret.data),
      origDataStr = JSON.stringify(
        JSON.parse(new TextDecoder().decode(origFile)),
      );
...
Deno.test(`ct=u, body=${SIMPLE_TEXT_BODY_ALPHABET}`, async () => {
  const req = await prepareRequest(
    HDR_VAL_UNKNOWN_TYPE,
    SIMPLE_TEXT_BODY_ALPHABET,
    undefined,
  );
  const ret = await parse(req);
...
Deno.test(`ct=audio/aac`, async () => {
  const ext = "aac";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.AUDIO_AAC,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/octet-stream`, async () => {
  const ext = "bin";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.OCTET_STREAM,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/x-bzip2`, async () => {
  const ext = "bz2";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.BZIP_2,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=text/csv`, async () => {
  const ext = "csv";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.CSV,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/msword`, async () => {
  const ext = "doc";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.MS_WORD,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.openxmlformats-officedocument.wordprocessingml.document`, async () => {
  const ext = "docx";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.MS_WORD_X,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/epub+zip`, async () => {
  const ext = "epub";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.EPUB,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/gzip`, async () => {
  const ext = "gz";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.GZ,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=image/gif`, async () => {
  const ext = "gif";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.GIF,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=text/html`, async () => {
  const ext = "html";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.HTML,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=image/jpeg`, async () => {
  const ext = "jpg";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.JPG,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/json`, async () => {
  const ext = "json";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.JSON,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=audio/mpeg`, async () => {
  const ext = "mp3";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.MP3,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=video/mp4`, async () => {
  const ext = "mp4";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.MP4,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=video/mpeg`, async () => {
  const ext = "mpeg";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.MPEG,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=image/png`, async () => {
  const ext = "png";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.PNG,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/pdf`, async () => {
  const ext = "pdf";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.PDF,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.ms-powerpoint`, async () => {
  const ext = "ppt";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.PPT,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.openxmlformats-officedocument.presentationml.presentation`, async () => {
  const ext = "pptx";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.PPT_X,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.rar`, async () => {
  const ext = "rar";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.RAR,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/rtf`, async () => {
  const ext = "rtf";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.RTF,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=image/svg+xml`, async () => {
  const ext = "svg";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.SVG,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/x-tar`, async () => {
  const ext = "tar";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.TAR,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=text/plain, body=${SIMPLE_TEXT_BODY_ALPHABET}`, async () => {
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.TEXT,
    SIMPLE_TEXT_BODY_ALPHABET,
    undefined,
  );
  const ret = await parse(req);
...
Deno.test(`ct=audio/wav`, async () => {
  const ext = "wav";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.WAV,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.ms-excel`, async () => {
  const ext = "xls";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.XLS,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, async () => {
  const ext = "xlsx";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.XLS_X,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/zip`, async () => {
  const ext = "zip";
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.ZIP,
    undefined,
    ext,
  );
  const ret = await parse(req);
...
Deno.test(`ct=application/x-www-form-urlencoded, body=1`, async () => {
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.URL_ENCODED,
    URL_ENCODED_DATA[1].a,
    undefined,
  );
  const ret = await parse(req);
...
Deno.test(`ct=text/xml, body=simple`, async () => {
  const req = await prepareRequest(
    ParserMeta.MIME_CONTENT_TYPES.XML,
    XML_SAMPLE,
    undefined,
  );
  const ret = await parse(req);
...
Deno.test(`ct=text/xml, body=sample, sbtf`, async () => {
  ...
  const ret = await parse(
    req,
    Object.assign({}, OPTIONS_SAVE_BODY_TO_FILE, OPTIONS_NO_XML_TO_JSON),
  );
  await fileAsserts(ret.files, LOCAL_DIR, ext, true);
});
...
Deno.test(`ct=text/xml, body=sample, sbtftp`, async () => {
  ...
  const ret = await parse(
    ...
    Object.assign(
      {},
      OPTIONS_SAVE_BODY_TO_FILE_TO_PATH,
      OPTIONS_NO_XML_TO_JSON,
    ),
  );
  await fileAsserts(ret.files, TEMP_DIR, ext, true);
});
...
// Deno.test(`ct=multipart/form-data, body=simple`, { only: true }, async () => {
//   const req = await prepareRequest(
//     ParserMeta.MIME_CONTENT_TYPES.MULTIPART_FORM_DATA,
//     SIMPLE_JSON_FOR_MFD,
//     undefined,
//   );
//   const ret = await parse(req);
...
//   const req = await prepareRequest(
//     ParserMeta.MIME_CONTENT_TYPES.MULTIPART_FORM_DATA,
//     Object.assign({}, SIMPLE_JSON_FOR_MFD, { exts }),
//     undefined,
//   );
//   const ret = await parse(req);
//   await dataAsserts(ret, JSON.stringify(SIMPLE_JSON_FOR_MFD));
